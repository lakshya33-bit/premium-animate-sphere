...
import { Search, ArrowRight, Eye, ExternalLink, X, Clock, CreditCard, Tag, Heart, Share2, Globe, ArrowUpRight, Gift } from "lucide-react";
...
function QuickView({ voucher, open, onClose }: { voucher: Voucher | null; open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="glass-card border-border/50 sm:max-w-xl max-h-[85vh] overflow-y-auto p-0">
        {/* Header */}
        <div className="p-6 pb-4 border-b border-border/30">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${voucher.color}15` }}>
                <Icon className="w-7 h-7" style={{ color: voucher.color }} />
              </div>
              <div>
                <DialogTitle className="font-serif text-2xl font-bold">{voucher.name}</DialogTitle>
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-secondary/60 text-muted-foreground">{voucher.category}</span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button className="px-4 py-2 rounded-xl glass-card text-xs font-medium text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5">
                <Share2 className="w-3.5 h-3.5" /> Share
              </button>
              <Link to={`/vouchers/${voucher.id}`} onClick={onClose} className="px-4 py-2 rounded-xl glass-card text-xs font-medium text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1.5">
                <Globe className="w-3.5 h-3.5" /> Full Details <ArrowUpRight className="w-3 h-3" />
              </Link>
            </div>
          </div>
        </div>
          ...
          {voucher.platformRates.map((pr, i) => (
            ...
            >
              ...
              <div className="w-12 h-12 rounded-xl bg-secondary/60 flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-bold text-foreground">{pr.platform.charAt(0)}</span>
              </div>
              ...
              <div className="text-right flex-shrink-0 mr-2">
                <p className="text-[10px] text-muted-foreground">{pr.type}</p>
                <p className="text-sm font-bold text-gold">{pr.savings}</p>
                {pr.bestCard && (
                  <span className="inline-flex items-center gap-1 text-[9px] px-2 py-0.5 rounded-full bg-gold/10 text-gold mt-0.5">
                    <CreditCard className="w-2.5 h-2.5" /> {pr.bestCard}
                  </span>
                )}
              </div>

              {/* Buy button */}
              <button className="px-5 py-2.5 rounded-xl bg-secondary/80 hover:bg-secondary text-xs font-medium text-foreground transition-colors flex items-center gap-1.5 flex-shrink-0">
        ...
        {/* Footer info */}
        <div className="px-6 pb-5 pt-2 flex items-center gap-4 text-[10px] text-muted-foreground">
          <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> Validity: {voucher.validity}</span>
          <span className="flex items-center gap-1"><Tag className="w-3 h-3" /> Best Rate: {voucher.bestRate}</span>
        </div>
      </DialogContent>
    </Dialog>
  );
