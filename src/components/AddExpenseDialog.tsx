...
export default function AddExpenseDialog({ cardId, cardName, cardColor, onAdd, trigger }: AddExpenseDialogProps) {
  ...
  const categoryLabel = CATEGORIES.find((c) => c.value === category)?.label || "Shopping";
        ...
        <div className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                <Input
                  ...
                  className="bg-secondary/30 border-border/20 text-sm h-11"
                />
              </div>
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={category} onValueChange={setCategory}>
                <SelectTrigger className="bg-secondary/30 border-border/20 text-sm h-11">
                  <SelectValue />
                </SelectTrigger>
          ...
          <div>
            ...
            <Textarea
              ...
              maxLength={200}
              rows={2}
              className="bg-secondary/30 border-border/20 text-sm resize-none"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              ...
              className="px-5 py-2.5 rounded-xl text-sm border border-border/30 hover:bg-secondary/30 transition-colors font-medium"
            >
              Cancel
            </button>
            <button
              onClick={handleSubmit}
              disabled={!merchant.trim() || !amount || parseFloat(amount) <= 0}
              className="px-5 py-2.5 rounded-xl text-sm gold-btn font-semibold shadow-md shadow-gold/10 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Add Expense
            </button>
          </div>
